...
func NewStats() *Stats {
	return &Stats{
		gamesCountEstimated: new(uint64),
		gamesCountElapsed:   new(uint64),
		gamesInProgress:     new(uint64),
		games: &StatsGames{
			played: new(uint64),
			won:    new(uint64),
			lose:   new(uint64),
...
func (s *Stats) Print(start time.Time) {
	p := 100 - float64(atomic.LoadUint64(s.gamesCountElapsed))/float64(atomic.LoadUint64(s.gamesCountEstimated))*100
